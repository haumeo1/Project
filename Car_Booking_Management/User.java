...
public class User 
{
  ...
  // method to decrease number of ride
  public void deRide(){
    this.rides--;
  }
  ...
  // method to decrease number of delivery
  ...
  {
    // check whether other is the instance of a particular class or interface of Class User
    if (!(other instanceof User)){
      return false;
    }
    // Transfer other to class User
    User user = (User)other;
   
    // check if 2 users are the same or not through their name and address
    return this.name.equalsIgnoreCase(user.name) && this.address.equalsIgnoreCase(user.address);
  }
}
