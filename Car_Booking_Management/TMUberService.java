...
abstract public class TMUberService 
{
  ...
  {
    // check whether other is the instance of a particular class or interface of Class TMUberService
    if (!(other instanceof TMUberService)){
      return false;
    }
    // Transfer other to class TMUberService
    TMUberService service = (TMUberService)other;

    if(!(this.getUser().equals(service.getUser()) && this.type.equals(service.type))){
      return false;
    }
    return true;
  }
